...
function cardGradients($item) {
    return htmlspecialchars($item['gradient_from'], ENT_QUOTES) . ' ' . htmlspecialchars($item['gradient_to'], ENT_QUOTES);
}
    ...
    * { font-family: 'DM Sans', sans-serif; }
    ...
    body {
      ...
      background-image:
        ...
        radial-gradient(ellipse at 80% 10%, rgba(59,130,246,0.1) 0%, transparent 55%);
      min-height: 100vh;
    }
    ...
    .fade-up-2 { animation: fadeUp 0.55s ease both; animation-delay: 0.18s; }
    .fade-up-3 { animation: fadeUp 0.55s ease both; animation-delay: 0.30s; }
    .fade-up-4 { animation: fadeUp 0.55s ease both; animation-delay: 0.42s; }
    .fade-up-5 { animation: fadeUp 0.55s ease both; animation-delay: 0.54s; }
    ...
    .weather-inner { transition: transform 0.35s cubic-bezier(.34,1.56,.64,1); }
...
<body class="min-h-screen">
  ...
  <header class="glass-header sticky top-0 z-50 px-4 sm:px-6 py-3.5">
    <div class="max-w-6xl mx-auto flex items-center justify-between gap-4">
      ...
      <div class="flex items-center gap-2.5 shrink-0">
        ...
        <span class="font-display font-700 text-gray-800 text-lg tracking-tight" style="font-family:'Syne',sans-serif;font-weight:700;">Outfit<span class="text-indigo-500">Planner</span></span>
      </div>
  ...
  <main class="max-w-6xl mx-auto px-4 sm:px-6 py-10 space-y-12">
    ...
    <section class="fade-up-1 weather-card-wrap">
      <div class="weather-inner weather-gradient rounded-3xl p-8 sm:p-12 shadow-2xl shadow-indigo-300/40 text-white text-center relative">
        ...
        <!-- Temp -->
        <p id="weatherTemp" class="font-display text-8xl sm:text-9xl font-extrabold leading-none tracking-tighter drop-shadow-sm" style="font-family:'Syne',sans-serif;"><?= h($weather['temperature']) ?>°</p>
      ...
      <section class="fade-up-2 lg:col-span-2">
        <div class="bg-white rounded-3xl shadow-lg shadow-slate-200/70 p-6 sm:p-8 ring-1 ring-slate-100 h-full">
          ...
          <div class="flex items-center gap-3 mb-7">
            ...
            <h2 class="font-display text-xl font-bold text-gray-800" style="font-family:'Syne',sans-serif;">Add New Item</h2>
          </div>
          ...
          <div class="space-y-4">
            ...
            <div>
              ...
              <select id="categorySelect" class="custom-input w-full px-4 py-2.5 pr-10 rounded-xl border border-gray-200 bg-gray-50 text-sm text-gray-700 transition-all duration-200 cursor-pointer">
                ...
                ?>
                <option value="<?= h($cat) ?>"><?= h($icon . ' ' . $cat) ?></option>
                <?php endforeach; ?>
              </select>
            </div>
            ...
            <div>
              ...
              <select id="seasonSelect" class="custom-input w-full px-4 py-2.5 pr-10 rounded-xl border border-gray-200 bg-gray-50 text-sm text-gray-700 transition-all duration-200 cursor-pointer">
                ...
                <?php foreach ($seasons as $s):
                    $icon = $seasonEmojis[$s] ?? '';
                ?>
                <option value="<?= h($s) ?>"><?= h($icon . ' ' . $s) ?></option>
                <?php endforeach; ?>
              </select>
            </div>
        ...
        <div class="bg-white rounded-3xl shadow-lg shadow-slate-200/70 p-6 sm:p-8 ring-1 ring-slate-100 h-full">
          ...
          <div class="flex items-center justify-between mb-7">
            <div class="flex items-center gap-3">
              ...
              <h2 class="font-display text-xl font-bold text-gray-800" style="font-family:'Syne',sans-serif;">Suggested Outfits</h2>
            </div>
            <span id="suggestedCount" class="text-xs font-semibold text-indigo-500 bg-indigo-50 px-3 py-1 rounded-full"><?= count($suggestedItems) ?> item<?= count($suggestedItems) !== 1 ? 's' : '' ?></span>
          </div>

          <!-- Grid (populated by JS on load; PHP renders initial state too) -->
    ...
    <section class="fade-up-4">
      <div class="flex items-center justify-between mb-6">
        <div>
          <h2 class="font-display text-2xl font-bold text-gray-800" style="font-family:'Syne',sans-serif;">Full Wardrobe</h2>
      ...
      <div id="wardrobeGrid" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
        ...
        <!-- "No more outfits" placeholder always shown -->
  ...
  <div id="editModal">
    <div class="bg-white rounded-3xl shadow-2xl p-8 w-full max-w-sm mx-4 ring-1 ring-slate-100">
      <div class="flex items-center justify-between mb-6">
        <h3 class="font-display text-xl font-bold text-gray-800" style="font-family:'Syne',sans-serif;">Edit Item</h3>
