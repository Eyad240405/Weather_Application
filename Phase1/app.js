...
document.addEventListener('DOMContentLoaded', () => {
    ...
    loadWeather().then(() => {
        loadWardrobe().then(() => updateSuggestedByWeather());
    });
});
...
function bindEvents() {
    DOM.citySearchInput?.addEventListener('keydown', async (e) => {
        ...
        if (!city) { showToast('Please enter a city name.', false); return; }
    ...
    DOM.photoInput?.addEventListener('change', () => {
        ...
        if (DOM.photoLabel) DOM.photoLabel.textContent = file ? file.name : 'Click to upload';
    });
...
async function loadWeather(city = 'Cairo') {
    ...
    if (result.success || result.fallback) {
        ...
        // If the server updated suggested outfits in the DB, reload from DB
        if (result.suggested_updated) {
            const suggested = await API.getSuggestedOutfits();
            if (suggested.success) renderSuggested(suggested.items || []);
        }
    } else {
...
function renderSuggested(items) {
    ...
    const slots = 4;
    let html = items.slice(0, slots).map(buildSuggestedCard).join('');
    for (let i = items.length; i < slots; i++) html += buildSkeletonCard();
    grid.innerHTML = html;
...
function buildSuggestedCard(item) {
    ...
    const imgContent  = item.image_path
        ? `<img src="${esc(item.image_path)}" alt="${esc(item.name)}" class="w-full h-full object-cover"/>`
        : `<div class="h-36 bg-gradient-to-br ${esc(item.gradient_from)} ${esc(item.gradient_to)} flex items-center justify-center text-5xl select-none">${esc(item.emoji)}</div>`;
...
<div class="outfit-card bg-gray-50 rounded-2xl overflow-hidden ring-1 ring-slate-100 cursor-pointer relative" data-id="${item.id}">
  <div class="relative overflow-hidden">
    ...
    <div class="card-overlay absolute inset-0 bg-indigo-600/80 flex items-center justify-center gap-3">
      <button class="card-edit  w-8 h-8 rounded-full bg-white/20 hover:bg-white/30 flex items-center justify-center text-white transition-colors">${iconEdit()}</button>
      <button class="card-delete w-8 h-8 rounded-full bg-red-500/80 hover:bg-red-500 flex items-center justify-center text-white transition-colors">${iconDelete()}</button>
    </div>
  </div>
  <div class="p-3.5">
    <p class="font-bold text-gray-800 text-sm leading-tight">${esc(item.name)}</p>
    <p class="text-xs text-gray-400 mt-0.5">${esc(item.category)}</p>
    <span class="mt-2 inline-block text-xs font-semibold ${badgeClass} px-2.5 py-0.5 rounded-full">${seasonEmoji} ${esc(item.season)}</span>
  </div>
</div>`;
}
...
<div class="col-span-full flex flex-col items-center justify-center py-20 text-center">
  ...
  <p class="text-sm text-gray-400 mt-2">Start by adding your clothes using the form on the left.</p>
    ...
    grid.innerHTML = items.map(buildWardrobeCard).join('') + buildNoMoreCard();
    attachCardActions(grid);
}
...
function buildWardrobeCard(item) {
    ...
    const imgContent  = item.image_path
        ? `<img src="${esc(item.image_path)}" alt="${esc(item.name)}" class="w-full h-full object-cover"/>`
        : `<div class="h-44 bg-gradient-to-br ${esc(item.gradient_from)} ${esc(item.gradient_to)} flex items-center justify-center text-6xl select-none">${esc(item.emoji)}</div>`;
...
<div class="outfit-card bg-white rounded-2xl overflow-hidden shadow-md shadow-slate-200/60 ring-1 ring-slate-100 cursor-pointer relative" data-id="${item.id}">
  <div class="relative overflow-hidden">
    ${imgContent}
    <div class="card-overlay absolute inset-0 bg-indigo-600/80 flex items-center justify-center gap-3">
      <button class="card-edit  w-9 h-9 rounded-full bg-white/20 hover:bg-white/30 flex items-center justify-center text-white transition-colors">${iconEdit()}</button>
      <button class="card-delete w-9 h-9 rounded-full bg-red-500/80 hover:bg-red-500 flex items-center justify-center text-white transition-colors">${iconDelete()}</button>
    </div>
  </div>
  <div class="p-4">
    <p class="font-bold text-gray-800 text-sm">${esc(item.name)}</p>
    <p class="text-xs text-gray-400 mt-0.5 mb-2.5">${esc(item.category)}</p>
    <span class="text-xs font-semibold ${badgeClass} px-2.5 py-1 rounded-full">${seasonEmoji} ${esc(item.season)}</span>
  </div>
</div>`;
}

function buildNoMoreCard() {
    return `
...
function attachCardActions(grid, isSuggested) {
    ...
    // امنع تكرار ربط نفس الحدث أكثر من مرة
...
async function handleAddItem() {
    ...
    if (!name)     { showToast('Please enter an item name.', false);   return; }
    if (!category) { showToast('Please select a category.', false);    return; }
    if (!season)   { showToast('Please select a season.', false);      return; }

    let imagePath = null;

    if (file) {
        if (file.size > 8 * 1024 * 1024) {
            showToast('File too large. Maximum 8 MB.', false); return;
        }
        if (!['image/jpeg', 'image/png', 'image/gif', 'image/webp'].includes(file.type)) {
            showToast('Invalid file type. PNG, JPG, GIF, WEBP only.', false); return;
        }
        const uploadResult = await API.uploadClothing(file);
        if (!uploadResult.success) { showToast(uploadResult.message, false); return; }
...
function showToast(message, success = true) {
    ...
    t.className   = `fixed bottom-5 right-5 z-[9999] px-4 py-3 rounded-xl shadow-lg text-white text-sm font-medium
        ${success ? 'bg-emerald-500' : 'bg-rose-500'} opacity-0 translate-y-3 transition-all duration-300`;
    document.body.appendChild(t);
    requestAnimationFrame(() => t.classList.remove('opacity-0', 'translate-y-3'));
    setTimeout(() => {
...
function iconEdit() {
    return `<svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
